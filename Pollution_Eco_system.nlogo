globals [
        max-health ; max health of one person
        median-health ;  eventually track global health
        ;avg-exposure ; eventually track global exposure
        ;avg-contamination ; eventually track global contamination
        pollution
        ]
...
people-own [exposure health]

;;;;;;;;;;;; SETUP ;;;;;;;;;;;;;;;;;
to setup
  clear-all
  set max-health 50
  set median-health max-health
  setup-people
  setup-factories
  setup-cleaners
  reset-ticks
...
to setup-people ; people procedure
  create-people 100
...
to setup-factories ; factory procedure
  create-factories num-factories
  ask factories [set color red
                 set size 2 ]
  ask factories [setxy random-xcor random-ycor]
  ask factories [set shape "box"]
end

to setup-cleaners ; cleaners procedure
  create-cleaners num-cleaners
  ask cleaners [set color green]
 ; ask cleaners [setxy random-xcor random-ycor]
 ; ask cleaners [ move-to max-one-of  factories [contamination ]]
 ; ask cleaners [right random-float 5 forward 2 ]
  ask cleaners [set shape "leaf"]
 ifelse (random-cleaner-localization?)
    [ask cleaners [setxy random-xcor random-ycor]]
    [ask cleaners [ move-to max-one-of  factories [contamination ] ]]
end
...
;;;;;;;;;;;;; RUN Procedures ;;;;;;;;;;;;;;;;;
to move-people; people procedure
  ask people [ right random-float 50 lt random-float 50 forward 1
              set health health - 0.01]
end

to factory-pollute ; factory procedure
  ask factories [set contamination (contamination + 20)] ; factory generate contamination
  diffuse contamination 0.10 ; spread of contamination
  ;; cleaners reduce contamination around neighbors ==>
  ask cleaners [ask neighbors [set contamination contamination - contamination * clean-rate]]
end

to check-exposure ; people procedure ==> exposure rate
  ask people [if contamination > 30 or contamination < 50
      [set exposure (exposure + contamination * 0.50)]
  ask people [if contamination > 50
      [set exposure (exposure + contamination * 0.75)]
  ]]
end

to check-health ; people procedure
  ask people
      [if random-float  100 > int(exposure / 100)
                               [set health health - int(exposure / 100)
                                if health > max-health  [ set health max-health ]
                                ]]
end

to maybe-die  ;; people procedure
  ask people [ if health <= 0 [ die ]]
end

to reproduce  ;; people procedure
 ask people [
  if random-float 100 < birth-rate and health > max-health * 0.75   ;; throw "dice" to see if you will reproduce
  [ set health (health / 2 )  ;; divide health between parent and offspring
      hatch 1 [ rt random-float 360  fd 1] ]
  ]
end

;to update-global-pollution ;pathces
  ;set pollution (count factories with [contamination > 0] / (count factories))

;end

to update-viz ; pathces
  ask patches with [contamination >= 10] [set pcolor yellow]
  ask patches with [contamination >= 50] [set pcolor orange]
end

to update-global-pollution
  set pollution (count patches with [contamination > 50] / (count patches) * 100)
end

to regeneration
  ask patches with [ contamination <= 10]
  [
    if random-float 100 < (resource-regeneration / 10 )
      [ set contamination contamination - birth-rate * 10  ]] ; as more people => slower decontamination
end

to update-global-health
  ask people [set median-health median (list (health)) ]
end
...
"" ""
PENS
"people" 1.0 0 -13840069 true "" "plot count people"
"pollution" 1.0 0 -955883 true "" "plot pollution"
"health" 1.0 0 -13791810 true "" "plot median-health"
